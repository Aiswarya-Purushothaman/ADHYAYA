...
import {  updateEmailinst } from "../../utils/Axios/api";
import { toast } from "react-toastify";
import { useNavigate } from "react-router-dom";
import { useSelector } from "react-redux";

const UpdateEmailnst = () => {
  ...
  const EmailHandle = async (e: any) => {
    ...
    try {
      const data = { email };
      console.log(instructorInfo.email,email,"ghjhdsjksfd");
      if(instructorInfo.email===email){
        toast.error("Enter New email")
        return
      }
      
      const response = await updateEmailinst({ email });
      ...
      navigate("/instructor/otp-emailupdateInst");
    } catch (error: any) {
