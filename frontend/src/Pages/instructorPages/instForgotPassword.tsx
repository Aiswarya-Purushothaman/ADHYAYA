...
const InstForgotPassword = () => {
  ...
  const ForgotHandle = async (e: any) => {
    ...
    try {
      const data = { email };
      const response = await ForgotOtpinstructor({ email });
      console.log(response.data, "returned otp response");
      localStorage.setItem("otpforgotinst", response.data);
      localStorage.setItem("datainst", data.email);
      navigate("/instructor/otp-forgotinst");
    } catch (error: any) {
...
export default InstForgotPassword;
