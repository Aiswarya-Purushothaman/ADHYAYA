...
const ForgotPassAdmin = () => {
  ...
  const ForgotHandle = async (e: any) => {
    ...
    try {
      const data = { email };
      const response = await ForgotOtpAdminFunction({ email });
      console.log(response.data, "returned otp response");
      localStorage.setItem("otpforgotadmin", response.data);
      localStorage.setItem("dataadmin", data.email);
      navigate("/admin/otp-forgotadmin");
    } catch (error: any) {
