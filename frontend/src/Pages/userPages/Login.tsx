...
import { loginUser } from "../../utils/Axios/api";
...
const Login = () => {
  ...
  const GoogleLoginCall=async (res:any)=>{
    try {
      ...
      const response=await loginUser({email:email,googleUserId:googleUserId})
      console.log(response,"responseresponseresponseresponseresponse")
      if (response.data,"responseresponse") {
        dispatch(setUserDetails(response.data));
        navigate("/");
        toast.success("Login successful");
      } else {
  ...
  return (
    <>
      <div className="bg-white flex items-center justify-center min-h-screen bg-orbit1 p-4">
        <div className="flex flex-col md:flex-row w-full max-w-4xl ">
          ...
          <div className="bg-gradient-to-br from-red-600 to-pink-500 text-white p-4 md:w-1/2 w-full flex items-center justify-center">
            <div className="flex flex-col text-center items-center justify-center">
              <h1 className="text-4xl font-bold">Welcome To Sign In</h1>
              <p className="text-1xl">Don't Have An Account?</p>
              <Link to={"/register"} className="mt-4 border-r px-4 py-2 bg-gradient-to-br from-red-600 to-pink-500 font-medium text-white rounded-full hover:text-black transition duration-300 border-2">
                Signup
              </Link>
            </div>
          </div>
