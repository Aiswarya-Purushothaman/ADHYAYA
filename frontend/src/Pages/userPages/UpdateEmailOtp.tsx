...
import { ForgotOtpStudent, updateEmailUser } from "../../utils/Axios/api";
...
const throwError=useError()
  ...
  const handleOtpSubmit = async (e: React.FormEvent) => {
    try {
      ...
      const storedOtp = localStorage.getItem("otpupdate");
  console.log(enteredOtp,storedOtp,"otps");
  
      if (enteredOtp === storedOtp) {
        console.log("OTP is a match");
        localStorage.removeItem("otpupdate");
        const data =localStorage.getItem("data")
        console.log(data,"datadatadata");
        if(data){
  
          const email=data
          const response=await updateEmailUser({email:email})
  if(response.data){
...
console.log(userDetails,"userDetailsuserDetails");
    ...
    if(userDetails){
      ...
      const otp=await ForgotOtpStudent({email})
      console.log(otp.data,"otpotp");
      localStorage.setItem("otpforgot",otp.data)
    }
  };
