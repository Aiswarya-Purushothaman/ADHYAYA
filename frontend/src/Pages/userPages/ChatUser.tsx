...
const ChatUser = () => {
  ...
  const [currentUser, setCurrentUser] = useState<Instructor | null | any>(null);
  ...
  const handleDeleteChat = async () => {
    if (selectedChatId) {
      try {
        await deleteUserChat({ ChatId: selectedChatId });
        toast.success("Chat deleted ");
        setMessages((prevMessages) =>
          prevMessages.filter((message) => message._id !== selectedChatId)
        );
      } catch (error: any) {
  ...
  useEffect(() => {
    const instructors = async () => {
      try {
        const response = await fetchMyInstructors();
        if (response?.data) {
  ...
  useEffect(() => {
    const Messages = async () => {
      try {
        ...
        if (receiverId) {
          const response = await fetchUserMessages({ receiverId: receiverId });
          setMessages(response?.data);
          console.log(messages, "usermessaegs");
        }
      } catch (error: any) {
        const message = error?.response?.data?.message;
        console.log(error, "erorrorororr");
  ...
  useEffect(() => {
    ...
    socket.on("Authenticated", async (user: any) => {
      ...
      users?.forEach((instructor: any) => {
        const room = `room-${user.user._id}-${instructor._id}`;
        socket.emit("JoinRoom", room);
      });
    });

    // socket.on("userOnline", (data) => {
    //   console.log(data, 'online data student');
  ...
  const handleSendMessage = () => {
    if (currentMessage.trim()) {
      const room = `room-${sender._id}-${currentUser?._id}`;
      socket.emit("Message", {
        type: "text",
    ...
    if (file) {
      ...
      Axios.post("https://api.cloudinary.com/v1_1/dixhsgyfj/upload", formData)
        .then((response) => {
          ...
          // setMessages([...messages, { type, content: url }]);
        })
        .catch((error) => {
  ...
  return (
    <>
      ...
      <div className="flex h-screen border ">
        <div className="col-span-1 w-1/3 p-2 bg-gradient-to-br from-red-600 to-pink-500 overflow-y-auto chat">
                    ...
                    <div className="flex gap-2 relative">
                      ...
                      >
                        ...
                        {msg?.sender && (
                          <>
                            <button
                              className="absolute top-0 right-0 mt-1 mr-1 text-gray-600 focus:outline-none"
                    ...
                    <div className="flex gap-2 relative">
                      ...
                      >
                        ...
                        {msg?.sender && (
                          <>
                            <button
                              className="absolute top-0 right-0 text-xl p-5 text-gray-600 focus:outline-none"
                              onClick={() => setShowMenu(msg._id)}
                            >
                              &#x22EE;
                            </button>
